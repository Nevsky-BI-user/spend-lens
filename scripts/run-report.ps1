# run-report.ps1
# Daily pipeline for spend-lens v1.2 email PDF digests:
#   1. collector collect (refreshes web/public/data/usage.json + Supabase push if configured)
#   2. report --type daily
#   3. on the 1st day of a month: also report --type monthly
#   4. on January 1st: also report --type yearly
...
try {
    ...
    # 4. Yearly report on January 1st
