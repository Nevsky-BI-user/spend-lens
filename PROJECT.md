...
### 6.4 Жодних видимих терміналів

**Ніщо в проєкті не відкриває термінал явно.** Ані за розкладом, ані по кліку: усе фонове виконується прихованим процесом. Це правило записане в [CONTRACT.md](CONTRACT.md) («Process launch policy») і зобовʼязує однаково людину й агента.

- Обидва завдання Планувальника запускають `powershell.exe` з `-WindowStyle Hidden`.
- Оновлення на вимогу — `scripts\refresh.vbs` через `wscript.exe` (не консольний хост), стиль вікна `0`. Подвійний клік по `.cmd` неминуче блимає консоллю на частку секунди — саме тому точка входу тепер `.vbs`.
- Усі дочірні процеси Node запускаються з `windowsHide: true`, браузер для PDF — з `--headless=new`.
- В операційних скриптах немає `pause`, `Read-Host` і `timeout` «щоб устигнути прочитати»: у прихованому запуску читати нікому, а блокуючий запит підвішує завдання (24.08.2026 саме так лишилися висіти три оболонки — 45 хвилин).
- Замість консолі: логи `collector/.cache/*.log`, підпис «Дані оновлено» у дашборді й PDF, кнопка «Оновити». Помилку, яку не можна проґавити, показує діалогове вікно — але не термінал.
