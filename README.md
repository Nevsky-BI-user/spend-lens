...
Правило перевіряється автоматично: `node scripts/window-lint.mjs` (17 правил), той самий лінтер у CI на кожен push і PR, плюс PreToolUse-хук для агента. Ті самі правила можна прогнати по будь-якому іншому проєкту: `node scripts/window-lint.mjs --root <тека>`. Деталі — [CONTRACT.md → Process launch policy](CONTRACT.md) і [PROJECT.md §6.4](PROJECT.md).

**Якщо вікно все-таки блимає** — подвійний клік на `scripts\tasks.vbs`. Аудит нічого не відкриває і нічого не вимикає: він показує, які завдання Планувальника взагалі здатні намалювати вікно, які повторюються частіше ніж раз на годину і що реально спрацьовувало останні години (журнал `Microsoft-Windows-TaskScheduler/Operational`) — тож «щось блимнуло щойно» отримує конкретну назву. Результат — у діалозі, що сам закривається, і в `collector\.cache\task-audit.log`. Щоб перевести завдання на беззвіконний принципал: `wscript.exe scripts\tasks.vbs fix` — два завдання spend-lens перереєструються, а будь-яке завдання `claude-*` (їх лишають по собі локальні сесії Claude Code) лагодиться на місці: змінюється лише принципал, розклад і дія залишаються (один раз на машину — виправлення коду не змінює вже зареєстроване завдання; докладніше — [RUNBOOK.md](RUNBOOK.md) §2).
